...
class Net_Classifier_cyc(nn.Module):
    def __init__(self, in_channels):
        ...
        # torch.nn.MaxPool2d(kernel_size, stride, padding)
        self.cnn = nn.Sequential(
            nn.Conv2d(in_channels, 64, (3, 3), stride=1,
                      padding=1),  # [64, 128, 128]
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d((2, 2), 2, 0),  # [32, 64, 64]

            nn.Conv2d(64, 128, (3, 3), stride=1, padding=1),  # [64, 64, 64]
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d((2, 2), 2, 0),  # [256, 32, 32]

            nn.Conv2d(128, 256, (3, 3), stride=1, padding=1),  # [64, 32, 32]
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.MaxPool2d((2, 2), 2, 0),  # [64, 16, 16]

            nn.Conv2d(256, 512, (3, 3), stride=1, padding=1),  # [64, 16, 16]
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.MaxPool2d(2, 2, 0),  # [64, 8, 8]

            nn.Conv2d(512, 512, (3, 3), stride=1, padding=1),  # [64, 8, 8]
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.MaxPool2d(2, 2, 0),  # [64, 4, 4]
        )
